...
1. From the checkout being checked (main checkout or your worktree), with `export PATH=$HOME/.cargo/bin:$PATH`:
   - Rust: `BASE=<base> scripts/gates.sh`. `<base>` is `main` for a task branch; pass `MUTANTS=0` only when the human asked for a quick run or on `main` itself (stage finish). Never raise `CARGO_BUILD_JOBS` above 3, never set a `TMPDIR` inside the repository.
