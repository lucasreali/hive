...
CI (`.github/workflows/ci.yml`, `macos.yml`; mutants sharded by `mutants.yml`) is authoritative; this is the local run, with the same mutants setup (`.cargo/mutants.toml`, needs `cargo-nextest`) in one shard. Heavy gates have taken the whole WSL VM down before, with the human's terminals: never widen the build or run gates in parallel with another checkout's gates.
