...
4. **Git:** one branch per task (`task/<id>-<slug>`, e.g. `task/0.2-protocol`), created from `main`, with small commits. Never push, never force, never rewrite history.
   - **When the task is finished** (every gate green, task ticked in `TODO.md`, report written), the agent merges its branch into `main` (`git merge <branch>` from `main`: a fast-forward when possible, otherwise a merge commit) and deletes the branch with `git branch -d <branch>`. Everything the agent did must end up on `main`, with no task branch left behind.
   - **Each agent owns its work, conflicts included.** Agents may work in parallel, each in its own git worktree; never touch another agent's worktree or branch. If `main` moved, merge `main` into your task branch (`git merge main`), resolve every conflict yourself while keeping the other work intact, run every gate again, then merge into `main`. Never rebase, never force.
