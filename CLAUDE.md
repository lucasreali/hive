...
4. **Git:** work directly on `main` with small commits. Create a branch (`task/<id>-<slug>`, e.g. `task/0.2-protocol`, from `main`) only for a very complex task that may conflict with other agents working in parallel (e.g. a `/stage` run with worktrees). Never force, never rewrite history. **Push only task branches**, always through `scripts/ci.sh push` (personal token, `task/*` only; allowed in `.claude/settings.json`), so CI runs the heavy gates; never push `main` or tags (the human's).
...
- **Push denied (403, `Permission to lucasreali/hive.git denied to lucasreali-visusai`):** `gh` is the git credential helper and has two accounts; the active one is the work account. Never switch accounts (other repos use the work one); `scripts/ci.sh push | watch | logs <run-id>` passes the personal token to each command. `main` and tags are pushed by the human: `gh auth switch -u lucasreali; git push origin main <tag>; gh auth switch -u lucasreali-visusai`.
