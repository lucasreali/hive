...
| `.claude/settings.json` + `hooks/` | Hooks: `guard.py` blocks edits to read-only files, npm/npx/pnpm/yarn, any push but `task/*` branches, rebase, force and global git config; `format.sh` runs `cargo fmt` / biome after each edit |
...
4. **Git:** work directly on `main` with small commits. Create a branch (`task/<id>-<slug>`, e.g. `task/0.2-protocol`, from `main`) only for a very complex task that may conflict with other agents working in parallel (e.g. a `/stage` run with worktrees). Never force, never rewrite history. **Push only task branches** (`git push -u origin task/<id>-<slug>`, with the personal token below) so CI runs the heavy gates; never push `main` or tags (the human's).
...
- **Push denied (403, `Permission to lucasreali/hive.git denied to lucasreali-visusai`):** `gh` is the git credential helper and has two accounts; the active one is the work account. Never switch accounts (other repos use the work one); pass the personal token to the one command instead: `GH_TOKEN=$(gh auth token -u lucasreali) git push -u origin task/<id>-<slug>` and `GH_TOKEN=$(gh auth token -u lucasreali) gh run watch <id> --exit-status`. `main` and tags are pushed by the human: `gh auth switch -u lucasreali; git push origin main <tag>; gh auth switch -u lucasreali-visusai`.
