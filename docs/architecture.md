...
| `hive::paths` | Runtime dir (`$XDG_RUNTIME_DIR/hive`, or `/tmp/hive-<uid>`, checked to be ours and mode 0700), socket, lockfile, `daemon.log`, data dir, bin dir, hooks settings, config dir (`$XDG_CONFIG_HOME/hive`, or `~/.config/hive`) and its `settings.json`. |
...
| `hive::terminal` | Spawns `fish -C 'set -gx PATH <bin> $PATH'` on a PTY with `HIVE_TERMINAL_ID` and its space's environment entries. Handles input and resize, and ends process groups. |
...
| `hive::transcript` | A subagent's conversation (6.10): its transcript's path beside its agent's, checks (agent id, inside Claude's projects folder), record parsing into entries and the tail read as it grows. |
...
6. The `terminal.scrollback` setting (default 5000 lines, see [Settings](#settings)) is applied to each new terminal.
...
2. On macOS every Ctrl+Shift+letter shortcut is Cmd+Shift+letter instead (`commandKey` in `src/window.ts`, from the user agent); Ctrl+Shift+letter then goes to the terminal. Tooltips and kbd chips show `⇧⌘B` there (`keyText`).
...
4. The store keeps the last `settings` as `settings` (`DEFAULT_SETTINGS` in `src/store.ts` until they arrive) and a `settings_failed` message as `settingsError` (cleared by the next `settings`), also shown in the status bar. The app applies `terminal.scrollback` to new terminals, `notifications.volume` to the tone and `agents.confirm_close` to the close guard.
...
4. UI: a select at the left of the sidebar header lists the spaces plus "New space…" and "Edit space…" (the dialog `SpaceDialog`: name, Claude config folder, git name, git email, GitHub CLI config folder; "Delete space" only enabled for an empty one). The sidebar shows the current space's projects (`spaceProjects`); the Sessions panel lists again when the space changes. The pending bell, F8, tones and OS notifications cover every space; a notification names the agent's space when there are several (`agentPlace` in `src/notify.ts`), and going to an agent of another space (`goToAgent` in `src/shortcuts.ts`, used by F8) sends `select_space` for it.
