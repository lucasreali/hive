...
| `chat_opened {chat, cwd, session, model, mode, commands, api_key_source}` | service → app | n | The chat's `claude` started (it answered `initialize`); `chat` = n; the session and model follow in `chat_status` with the first turn (`system/init`), and `api_key_source` stays null until 7.3i. `session` is its session id, `commands` the slash commands for the composer's `/` list, `api_key_source` set when it runs on an API key rather than the subscription login. |
...
| `error {message}` | service → client | 0 or n | A refused request, e.g. channel 0, a channel already open, a bad cwd, an unexpected message, or a second app; a chat message for a channel with no chat ("no chat is open on channel n"), a `confirm_chat_folder` nobody waits for, or a `chat_answer` (until 7.3e: "no such pending request"). |
...
### Chat in the service (7.3c)
1. `open_chat` on channel n (the chat's id): channel 0 or a channel held by a terminal or a chat is an `error`; `cwd` must be exactly a worktree of an added project and `resume` a UUID, else `chat_closed {error}`.
2. The first chat in a project waits for the human: `confirm_chat_folder {chat, cwd}`; `accepted: true` sets `projects.<id>.chat_confirmed` in the settings (the app gets `settings` again), `false` or `close_chat` answers `chat_closed {error: null}`. Later chats in the project start at once.
3. The service runs the `claude` the wrapper would run (`wrapper::real_claude` on the service's `PATH`) with separate arguments: `-p --input-format stream-json --output-format stream-json --verbose --replay-user-messages --permission-prompt-tool stdio --permission-mode <default|acceptEdits|plan> --settings <data>/hive/hive-hooks.json [--resume <id>]`, in the worktree, in a process group of its own, with a terminal's environment (its space's entries and its worktree's `HIVE_*`), `HIVE_TERMINAL_ID=n` and `HIVE_WRAPPED=1`, without `CLAUDECODE`/`CLAUDE_CODE_ENTRYPOINT`. Its hooks reach the service as a terminal's, so its agent is detected on channel n. Nothing found or a failed start: `chat_closed {error}`.
4. The first stdin line is the `initialize` control request; its answer sends `chat_opened` (commands from the answer). Each `chat_send` is a `user` line (text ≤ 1 MiB; images refused until 7.3g, as an `error` entry) and a `user` entry; `chat_interrupt` an `interrupt` request; `chat_set_mode` a `set_permission_mode` request.
5. stdout is read in lines of at most 16 MiB (a longer one is skipped and noted); each line is JSON or ignored. `assistant` blocks become `assistant`, `thinking` and `tool` entries (tool: one-line summary ≤ 500 characters, `running`); a `tool_result` re-sends its call's entry (`ok`/`error`, output); a subagent's messages carry `parent`; `system/init` sets session, model and mode; `system/status`, `api_retry` set `compacting`, `retry`; `compact_boundary` and `conversation_reset` are dividers; `informational` a note; `result` an error or "Interrupted" note plus a `usage` entry (time, output tokens, context %), and the end of the turn for the agent's state (`TurnFinished` or `TurnFailed`, as hooks do); a rejected `rate_limit_event` an error. Our own replayed turns, `stream_event` (7.3h) and unknown types are ignored. Every text is cut at 64 KiB; `chat_entries` carry ≤ 200 entries and ≤ 3 MiB. `chat_status` goes out whenever busy, mode, model, retry, compacting or session changed.
6. `can_use_tool` requests are denied (with a note) until 7.3e; other requests to the service get an error response.
7. `close_chat` closes stdin, then sends SIGINT, SIGTERM and SIGKILL to the group, each after 5 s if it is still there. When stdout ends the service reaps claude, removes its agent (`agent_removed`) and sends `chat_closed`, with the last 4 KiB of stderr (or the exit status) when it failed and the app had not closed it. When the service ends, every chat's stdin closes and its group gets the same signals 0.5 s apart.
