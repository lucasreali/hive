...
| `agent_state {id, state, urgency, pending, interrupted, subagents, activity, since_ms}` | service → app | n | The agent's displayed state, its urgency (0 ended … 8 waiting for permission; higher wins), whether it is pending (needs the user), whether it waits for you because the user interrupted it (nothing alerts), and its live subagents `[{id, agent_type, state, worktree, activity, since_ms}]`, `worktree` being the id of the subagent's own worktree or null; `activity` is what the agent (or subagent) itself is doing, or null, and `since_ms` the wall clock time (ms since the epoch) its displayed state (a subagent's: its state) began (see [Agent states](#agent-states)). Sent when it changes, after `agent_detected`, and for every live agent right after `welcome`. |
...
| `create_file {worktree, folder, name}` | app → service | 0 | Create the empty file `name` in `folder` (relative to the worktree, `""` for its root; the path rules of `open_file`, resolved inside the worktree). `name` is one component: not empty, `.` or `..`, no `/`, `\` or NUL, at most 255 bytes. Never overwrites (`O_EXCL`, which also refuses a symlink). Answered by `file_created` (the app opens it as editable text unless that would drop unsaved edits) or `file_op_failed`. |
| `file_created {worktree, path}` | service → app | 0 | The file now exists at `path`, relative to the worktree (through a symlinked folder: where it resolves). |
| `rename_file {worktree, path, name}` | app → service | 0 | Rename the regular file `path` (not a folder, not following a symlink) to `name` (the rules of `create_file`) in the same folder, never over an existing entry: a hard link to the new name, then the old name removed (the link removed again if that fails). Answered by `file_renamed` or `file_op_failed`. |
| `file_renamed {worktree, path, to}` | service → app | 0 | `path` is now `to`; the open file, its text and its edit buffer follow. The tree follows through the watch's `files`. |
| `file_op_failed {worktree, message}` | service → app | 0 | Nothing was created or renamed; `message` shows as is in the file name dialog. |
...
| `open_chat {cwd, resume, mode}` | app → service | n ≥ 1 | Start a chat (see [Chat](#chat-73)) on channel n, allocated by the app like a terminal's: `claude -p` in the worktree `cwd`, resuming session `resume` (null: a new one), in permission `mode` (null: `default`). Answered by `chat_opened` (after `confirm_chat_folder` for the first chat in a project) or `chat_closed`. |
| `chat_opened {chat, cwd, session, model, mode, commands, api_key_source}` | service → app | n | The chat's `claude` started (`system/init`); `chat` = n. `session` is its session id, `commands` the slash commands for the composer's `/` list, `api_key_source` set when it runs on an API key rather than the subscription login. |
| `chat_send {chat, text, images}` | app → service | n | A user turn; `images` = `[{media_type, data}]`, `data` base64. |
| `chat_entries {chat, entries, replace_last}` | service → app | n | New or changed rows: `entries` = `[{id, kind, text, tool, parent, status, output, image}]`, `id` increasing within the chat (an entry with a known `id` replaces that row: a tool's result), `kind` one of `user`, `assistant`, `thinking`, `tool`, `error`, `note`, `divider`, `usage`; `tool` is a tool call's name, `parent` the `Agent` call (`parent_tool_use_id`) a subagent's entry belongs to, `status` `running`, `ok` or `error`, `output` a tool's result, `image` `{media_type, data}` or null. `replace_last`: the first entry replaces the app's last one (live text). |
| `chat_request {chat, request}` | service → app | n | A permission, question or plan waiting for the human: `request` = `{id, kind, tool, detail, reason, questions, plan}`, `kind` one of `permission`, `question`, `plan`; `detail` the full command, path or input JSON; `questions` = `[{question, header, multi, options[{label, description}]}]`. |
| `chat_answer {chat, request, answer}` | app → service | n | The answer to pending request `request` (its id): `answer` is `{kind: "allow"}`, `{kind: "deny", message}`, `{kind: "answers", answers}` (per question, the chosen labels or one free text), `{kind: "approve_plan", accept_edits}` or `{kind: "keep_planning", feedback}`. |
| `chat_request_gone {chat, request}` | service → app | n | The request was answered or cancelled; its card goes. |
| `chat_interrupt {chat}` | app → service | n | Stop the running turn. |
| `chat_set_mode {chat, mode}` | app → service | n | Switch the permission mode: `default`, `accept_edits` or `plan` (`bypassPermissions` is never offered). |
| `chat_status {chat, busy, mode, model, retry, compacting, session}` | service → app | n | `busy` while a turn runs; `retry` a transient API retry ("Retrying 2/10…") or null; `compacting` during a compaction. |
| `close_chat {chat}` | app → service | n | End the chat. Answered by `chat_closed`. |
| `chat_closed {chat, error}` | service → app | n | The chat's `claude` ended; `error` holds its last stderr lines when it failed. The channel is free again. |
| `confirm_chat_folder {chat, cwd, accepted}` | both | n | Service → app, without `accepted`: the first chat in this project waits for the human's confirmation (a `-p` session shows no workspace trust dialog). App → service: the answer. |
| `error {message}` | service → client | 0 or n | A refused request, e.g. channel 0, a channel already open, a bad cwd, an unexpected message, or a second app. Until 7.3c every app → service chat message is answered with "chat not available yet". |
...
|---|---|---|
| `disconnected {reason}` | app (Rust) → UI | The bridge exited or its stdout closed. `reason` is the bridge's stderr (at most 16 KiB), a protocol error, or "the hive bridge exited". Every open terminal gets `terminal_exited {code: null}` and every open chat `chat_closed {error: null}` first. Not sent after `version_mismatch`. |
...
`kind` is one of: `session_started`, `prompt_submitted`, `tool_started`/`tool_finished`/`tool_failed {tool}`, `permission_requested {tool}`, `notification {notification}`, `turn_finished`, `turn_failed {error}`, `subagent_started`, `subagent_stopped`, `compact_started` (`PreCompact`), `compact_finished` (`PostCompact`), `elicitation_requested` (`Elicitation`), `session_ended {reason}`, `worktree_created {name, path}`, `worktree_removed {path}`, or `other {event}`.
`notification` is one of `permission_prompt`, `elicitation_dialog`, `elicitation_url_dialog`, `elicitation_complete`, `elicitation_response`, `idle_prompt`, `agent_needs_input`, `quota_auto_resume_fired`, `quota_auto_resume_stale`, `quota_auto_resume_disabled`, or `other`.
`activity` is set on `tool_started` and `permission_requested` only: a short text from the payload's untrusted `tool_input` (`hive::adapter`): Edit/Write/MultiEdit/NotebookEdit "Editing <file>" and Read "Reading <file>", the file relative to the payload's `cwd` when inside it; Bash its `description`, else the first line of its `command`; Grep/Glob "Searching <pattern>"; Agent/Task its `description`; AskUserQuestion "Asking a question"; ExitPlanMode "Plan ready for approval"; any other tool, or a missing field, the tool's name. `compact_started` has the activity "Compacting". Control characters are dropped and it is cut at 120 characters.
...
### Chat (7.3)
The in-app chat runs `claude -p` in stream-json mode (design: `docs/spike/chat.md`); the service turns its stream into the `chat_*` messages above, and the app only renders them and sends the human's actions (#37). Until 7.3c the service answers every chat request with `error` ("chat not available yet"); the browser mock (`src/transport/mockChat.ts`) plays a scripted chat instead. The fixtures in `crates/hive/tests/fixtures/chat/*.jsonl` hold what `claude -p` prints in each scenario of `scripts/spike/record-chat.py`, from the documented shapes until the real recordings replace them.
1. `openChat(cwd, resume, mode)`: Rust picks the next channel (shared with terminals), registers it as a chat and sends `open_chat` on it. The call resolves with the channel id, which is the chat's id.
2. Every chat message travels on its chat's channel and carries the id as `chat` too. Messages for chats the UI did not open are dropped; `chat_closed` releases the channel. A reloaded UI's old chats get `close_chat`.
3. UI (7.3d): `openChat(cwd)` of `src/chats.ts` adds a tab of `kind: "chat"` (the tab bar's "+" → Agent, 7.5); the store keeps each chat's data in `chats[id]` (entries replaced by `id` or `replace_last`, at most 2000; pending requests; status; `confirm` while `confirm_chat_folder` waits; `closed`). The tab shows `ChatView` in place of the terminals: `ConversationView` (shared with the subagent view, 6.10; a subagent's entries grouped by `parent`) and `ChatComposer`. Chats take no part in a split and take no file references.
...
2. Entering waiting for permission, a plan or an answer, waiting for you or error plays a short tone synthesized with Web Audio (no audio file) at the `notifications.volume` setting (0 plays none); changes within 500 ms share one tone.
3. Working or with subagents → waiting for you is "agent finished": `showNotification` (`src/window.ts`) sends an OS notification through `tauri-plugin-notification` ("Agent finished", "project · worktree: waiting for you"), only when the message is `pending`: the service marks an agent that finished in view of the focused window as not pending (see [Agent states](#agent-states) item 5), and then there is no notification. The tone still plays. An `agent_state` with `interrupted` (the user interrupted the agent) raises nothing: no tone, inbox item or notification. The capability allows only `is_permission_granted`, `request_permission` and `notify`. Outside Tauri nothing is shown.
...
9. New file and rename (7.4): a right click on the files tree (or the Menu key on its active row) opens its menu: "New File…" in the folder, the file's folder, or the root below the rows; "Rename…" on a file still on disk. The dialog (`src/shell/FileMenu.tsx`, `openFileDialog(target, renaming)`) sends `create_file` / `rename_file` and shows `file_op_failed`'s message under the field.
...
1. A detected agent starts idle. Each later hook event with its `session_id` sets the state of the agent, or of the subagent named by `agent_id`: `SessionStart` idle; `UserPromptSubmit`, `PreToolUse`, `PostToolUse`, `PostToolUseFailure` (never error), `SubagentStart`, `PreCompact` and `Notification` `elicitation_complete`/`elicitation_response`/`quota_auto_resume_fired` working; `PermissionRequest` and `Notification` `permission_prompt` waiting for permission; `PreToolUse`/`PermissionRequest` of `AskUserQuestion`, `Elicitation` and `Notification` `elicitation_dialog`/`elicitation_url_dialog`/`agent_needs_input` waiting for your answer; `PreToolUse`/`PermissionRequest` of `ExitPlanMode` waiting for plan approval; `Stop` and `Notification` `idle_prompt`/`quota_auto_resume_stale`/`quota_auto_resume_disabled` waiting for you; `StopFailure` error; `SessionEnd` ended. `Notification` `permission_prompt` (Claude Code's reminder ~6 s into any dialog) leaves waiting for your answer or plan approval as it is. Anything else leaves the state as it is.
   - Compaction: the agent's own `PreCompact` shows working with the activity "Compacting" and remembers its state and activity; its `PostCompact` gives them back (another event in between forgets them). The `SessionStart` with `source: "compact"` of a detected session leaves the agent as it is (state, subagents, tokens); any other `SessionStart` of it detects it again.
   - A `Stop` whose `background_tasks` is not empty keeps waiting for you with the activity "N background tasks"; a `UserPromptSubmit` clears the activity.
...
3. Rule 1: `state` is the most urgent of the agent's own state, its subagents' states and "with subagents" when any subagent is live. Urgency, highest first: `waiting_permission`, `waiting_plan`, `waiting_answer`, `error`, `waiting_you`, `with_subagents`, `working`, `idle`, `ended`.
4. Rule 2 (interrupts; Esc/Ctrl+C fire no `Stop`, nor does declining a dialog): every second (the unhooked-claude tick) the service reads the new lines of the transcript of each agent that is working, with subagents or on a dialog (`transcript::Usage`, the same bounded reader as its tokens; the lines already there when the agent was detected are read first and never count). When the last main-conversation message read is `[Request interrupted by user…]` or a declined tool call (Claude Code's `toolUseResult` "User rejected tool use" on a record with a failed `tool_result`; declining with feedback writes another marker and Claude goes on; the result's text is the tool's, so it is never trusted), the agent and each subagent working or on a dialog (not those waiting on a background task) go to waiting for you, `interrupted` and not pending, until the displayed state changes. Fallback: when its terminal has printed nothing for `agents.silence_secs` (5 s by default, see [Settings](#settings)), counted from the later of the last output and the last hook event, the agent and each subagent that is working go to waiting for you. The three dialog states never decay with silence (a dialog is static on screen). Output alone never moves a state back; only hook events do.
5. Pending (the "N pending" counter and F8): waiting for permission, a plan or an answer, error and waiting for you. `urgency` is the state's rank in the order above; the app shows a collapsed project or worktree with the state of highest `urgency` inside and counts agents with `pending`, so it keeps no table of its own. Except (hive.md item 5): an agent whose displayed state goes from working or with subagents to waiting for you while its terminal is the one in `view` and the window has the focus is seen, so not pending, until its displayed state changes again (looking away later keeps it seen).
...
| `<data>/hive/hive-hooks.json` | 0644 | One exec-form hook per observed event (15 events, `hive hook <Event>`, 1 s) plus `WorktreeCreate` → `hive worktree hook-create` (60 s) and `WorktreeRemove` → `hive worktree hook-remove` (10 s). |
