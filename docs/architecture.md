...
| `hive::paths` | Runtime dir (`$XDG_RUNTIME_DIR/hive`, or `/tmp/hive-<uid>`, checked to be ours and mode 0700), socket, lockfile, `daemon.log`, data dir, bin dir, hooks settings, `ports.json`, config dir (`$XDG_CONFIG_HOME/hive`, or `~/.config/hive`) and its `settings.json`. |
...
| `hive::terminal` | Spawns `fish -C 'set -gx PATH <bin> $PATH'` on a PTY with `HIVE_TERMINAL_ID` and the worktree's `HIVE_*` entries. Handles input and resize, and ends process groups. |
