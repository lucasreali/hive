...
2. `watch_transcript` derives the subagent's transcript: `<transcript_path without .jsonl>/subagents/agent-<agent_id>.jsonl`, only for an `agent_id` of 1–64 characters in `[A-Za-z0-9_-]`. Each read canonicalizes it (links resolved) and reads it only when it is a regular file inside Claude's projects folder (`$CLAUDE_CONFIG_DIR/projects`, else `~/.claude/projects`).
